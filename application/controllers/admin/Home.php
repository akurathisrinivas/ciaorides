...
class Home extends CI_Controller {
    ...
    public function index() {
        $data['title'] = "Komodeo";
        $data['active_users_count'] = $this->loginmodel->active_users_count();
        $data['inactive_users_count'] = $this->loginmodel->in_active_users_count();
        $data['orders_count'] = $this->loginmodel->orders_count();
        $data['orders_count_completed'] = $this->loginmodel->orders_count('completed');
        $data['orders_count_accepted'] = $this->loginmodel->orders_count('accepted');
        $data['orders_count_can_us'] = $this->loginmodel->orders_count('cancelled by user');
        $data['orders_count_can_rider'] = $this->loginmodel->orders_count('cancelled by rider');
        $data['rides_count'] = $this->loginmodel->rides_count();
        $data['chats_count'] = $this->loginmodel->chats_count();
        $data['recent_bookings'] = $this->loginmodel->recent_bookings();
        if (!empty($_GET['from_date'])) {
            $from_date = $this->input->get_post('from_date') . ' 00:00:00';
            $to_date = $this->input->get_post('to_date') . ' 23:59:59';
            $data['stat_data'] = $this->get_totals($from_date, $to_date);
        } else {
        ...
        if (!empty($_GET['search_string'])) {
            ...
            $data['user_data'] = $this->get_userdetails($search);
    ...
    function get_userdetails($search) {
        $data['info'] = $this->db->where('mobile', $search)->get('users')->row();
        $user_id = $data['info']->id;
        $data['bookings'] = $this->db->where('user_id', $user_id)->get('orders')->result();
        $data['rides'] = $this->db->where('user_id', $user_id)->get('rides')->result();
        return $data;
    }
    ...
    function ciao_view($userid) {
        $data = array();
        $data['user_id'] = $userid;
        $data['chat_list'] = $this->db->where('user_id', $userid)->get('co_chat')->result();
        $update = $this->db->set('is_scean', 'yes')->where(['user_id' => $userid, 'type' => "user"])->update('co_chat');
        $this->load->view('admin/includes/header', $data);
        $this->load->view('admin/ciao_chats', $data);
        $this->load->view('admin/includes/footer', $data);
    }

    function admin_chat() {
        $data = array();
        $data['chat_list'] = $this->db->set("SET character_set_connection=utf8mb4")->where(['type' => 'user', 'is_scean' => 'no'])->order_by('id', 'desc')->get('co_chat')->result();
        $this->load->view('admin/includes/header', $data);
        $this->load->view('admin/ciao_chats_list', $data);
        $this->load->view('admin/includes/footer', $data);
    }
    ...
    function get_chat_list_api() {
        ...
        $data = $this->db->where('user_id', $user_id)->get('co_chat')->result();
        foreach ($data as $d) {
...
//                $this->registermodel->send_push_notification($this->input->get_post('message'), "Admin Notification", $fire);
            ...
            $data = $this->db->where('user_id', $this->input->get_post('user_id'))->get('co_chat')->result();
            echo json_encode($data);
            die;
        }
    }
